      ...
      <div className="max-w-6xl mx-auto relative">
        ...
        >
          <div className="inline-flex items-center gap-2 px-4 py-2 bg-[#214CE5]/10 border border-[#214CE5]/30 rounded-full mb-6">
            ...
            <span className="text-sm font-medium text-[#6B8EFF]">{t('badge')}</span>
          </div>
      ...
      <div className="max-w-6xl mx-auto">
        ...
        >
          <h2 className="text-4xl md:text-5xl font-black mb-4">
            {t('painTitle')?.split(' ').map((word, i, arr) => 
              i === arr.length - 1 
                ? <span key={i} className="text-[#214CE5]">{word}</span> 
                : word + ' '
            ) || 'Sound Familiar?'}
          </h2>
        </motion.div>
...
function FeaturesSection({ t }) {
  ...
  const features = [
    { icon: Layers, title: 'White-Label Everything', description: 'Your brand, your domain, your app. Clients never see WellPlan.', color: '#214CE5' },
    { icon: Users, title: 'Unlimited Sub-Accounts', description: 'Add clients without per-seat costs. Scale without limits.', color: '#06b6d4' },
    { icon: Bot, title: 'Done-For-You Fulfillment', description: 'Snapshots, automations, and funnels ready to deploy.', color: '#8b5cf6' },
    { icon: BarChart3, title: 'Agency Dashboard', description: "Bird's eye view of all clients, MRR, and performance.", color: '#10b981' },
    { icon: Shield, title: 'SaaS Mode', description: 'Bill clients monthly. Automated provisioning and billing.', color: '#f59e0b' },
    { icon: Rocket, title: 'Reseller Program', description: 'Earn recurring revenue on every client. Up to 40% margins.', color: '#ef4444' },
  ];
...
function PricingSection({ t }) {
  ...
  const plans = [
    { name: 'Agency Starter', setup: '$497', monthly: '$297', description: 'Perfect for new agencies', features: ['Up to 10 sub-accounts', 'White-label branding', 'All core features', 'Email support', 'Community access'], color: '#214CE5' },
    { name: 'Agency Pro', setup: '$997', monthly: '$497', description: 'For growing agencies', features: ['Unlimited sub-accounts', 'Custom domain', 'Priority support', 'API access', 'Dedicated onboarding', 'SaaS mode enabled'], popular: true, color: '#8b5cf6' },
    { name: 'Agency Enterprise', setup: 'Custom', monthly: 'Custom', description: 'For large agencies', features: ['Everything in Pro', 'Custom integrations', 'Dedicated account manager', 'Custom training', 'SLA guarantee', 'Volume discounts'], color: '#10b981' },
  ];
    ...
    <section ref={ref} className="py-24 px-4 sm:px-6 bg-[#0a0a0a]">
      <div className="max-w-6xl mx-auto">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={isInView ? { opacity: 1, y: 0 } : {}} className="text-center mb-16">
          <h2 className="text-4xl md:text-5xl font-black mb-4">{t('pricingTitle')}</h2>
          <p className="text-xl text-gray-400 max-w-2xl mx-auto">{t('pricingSubtitle')}</p>
        </motion.div>

        <div className="grid md:grid-cols-3 gap-6">
          {plans.map((plan, idx) => (
            <motion.div key={idx} initial={{ opacity: 0, y: 20 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ delay: idx * 0.1 }} className={`relative p-8 bg-white/[0.02] border rounded-2xl ${plan.popular ? 'border-[#214CE5]' : 'border-white/10'}`}>
              {plan.popular && <div className="absolute -top-4 left-1/2 -translate-x-1/2 px-4 py-1 bg-[#214CE5] rounded-full text-sm font-bold">Most Popular</div>}
              <div className="text-center mb-6">
                ...
                {plan.setup !== 'Custom' && <p className="text-sm text-gray-500 mt-1">{plan.setup} setup</p>}
              </div>
              ...
              <Link href="/demo" className={`block w-full py-3 text-center font-bold rounded-xl transition ${plan.popular ? 'bg-[#214CE5] hover:bg-[#1a3db8]' : 'border border-white/20 hover:bg-white/5'}`}>
                Get Started
              </Link>
            </motion.div>
          ))}
      ...
      <div className="max-w-4xl mx-auto">
        <div className="relative p-12 rounded-3xl overflow-hidden">
          ...
          <div className="relative text-center">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
              <Link href="/demo" className="px-8 py-4 bg-white text-[#214CE5] rounded-xl font-bold text-lg hover:bg-gray-100 transition flex items-center gap-2">
                {t('cta')}
                <ArrowRight className="w-5 h-5" />
              </Link>
