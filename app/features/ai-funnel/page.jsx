...
export default function FunnelAIPage() {
  const t = useTranslations('aiFunnelPage');

  const features = ['builder', 'landing', 'funnels', 'variants', 'mobile', 'brand'];
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 pt-24 sm:pt-32 pb-16 sm:pb-24">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center max-w-4xl mx-auto">
            ...
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-cyan-500/20 border border-cyan-500/30 mb-6">
              ...
              <span className="text-sm font-semibold text-cyan-400">{t('badge')}</span>
            </div>

            <h1 className="text-4xl sm:text-5xl md:text-7xl font-black mb-6 leading-tight">
              {t('headline')}{' '}
              <span className="bg-gradient-to-r from-cyan-400 to-blue-400 bg-clip-text text-transparent">{t('headlineHighlight')}</span>
            </h1>
            ...
            <div className="grid grid-cols-3 gap-4 sm:gap-8 max-w-lg mx-auto">
              ...
              <div className="text-center">
                ...
                <div className="text-xs sm:text-sm text-gray-500">{t('stats.templates')}</div>
              </div>
              <div className="text-center">
                <div className="text-3xl sm:text-4xl font-black text-white mb-1">0</div>
                <div className="text-xs sm:text-sm text-gray-500">{t('stats.code')}</div>
              </div>
            </div>
          </motion.div>
        </div>
      ...
      <section className="py-16 sm:py-24 border-y border-white/5">
        <div className="max-w-4xl mx-auto px-4 sm:px-6">
          <div className="text-center mb-12">
            <h2 className="text-3xl sm:text-4xl font-black mb-4">{t('demoTitle')} <span className="text-cyan-400">{t('demoHighlight')}</span></h2>
          </div>
          <div className="bg-[#111111] border border-white/10 rounded-2xl p-6 sm:p-8">
            <div className="mb-6">
              <p className="text-sm text-gray-500 mb-2">{t('demo.promptLabel')}</p>
              <div className="p-4 bg-white/5 rounded-xl">
                <p className="text-gray-300">"{t('demo.prompt')}"</p>
              </div>
            </div>
            <div className="flex items-center gap-2 mb-4">
              <Wand2 className="w-5 h-5 text-cyan-400" />
              <span className="text-sm text-cyan-400">{t('demo.generating')}</span>
            </div>
            <div className="aspect-video bg-gradient-to-br from-cyan-500/10 to-blue-500/10 rounded-xl border border-cyan-500/20 flex items-center justify-center">
              <div className="text-center">
                ...
                <p className="text-gray-400">{t('demo.result')}</p>
              </div>
            </div>
          </div>
        </div>
